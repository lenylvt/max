(() => {
  ...
  function createSearchBar() {
    ...
    input.addEventListener("input", () => {
      ...
      const answerEl = bar.querySelector(".ailens-search-answer");
    ...
    input.addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
        ...
        } else if (highlights.length === 0 && val) {
          askAI(val);
        } else {
  ...
  function performTextSearch(query) {
    ...
    // Highlight matches in reverse to avoid offset issues
  ...
  browser.runtime.onMessage.addListener((msg) => {
    if (msg.action === "triggerSearch") toggleSearchBar();
  });

  let cachedShortcut = null;
  loadShortcut("searchInPage", "Ctrl+Shift+F").then((s) => (cachedShortcut = s));
