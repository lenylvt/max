import { CLOUD_AUTH_URL } from "./config.js";
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const modeBYOKBtn = document.getElementById("mode-byok");
  ...
  const cloudSignedIn = document.getElementById("cloud-signed-in");
  const cloudEmailEl = document.getElementById("cloud-email");
  ...
  try {
    ...
    updateModeUI(currentSettings.useCloud !== false);
  ...
  // ── Cloud / BYOK Mode Toggle ──────────────────────────────────────────

  function updateModeUI(useCloud) {
    const modelSection = document.getElementById("model-section");

    if (useCloud) {
      modeCloudBtn.classList.add("toggle-btn-active");
      modeBYOKBtn.classList.remove("toggle-btn-active");
      cloudSection.style.display = "";
      byokSection.style.display = "none";

      // Hide model & temperature in Cloud mode (fixed server-side)
      if (modelSection) modelSection.style.display = "none";

      // Show signed in/out state
      if (currentSettings.cloudToken && currentSettings.cloudEmail) {
        cloudSignedOut.style.display = "none";
        cloudSignedIn.style.display = "";
        cloudEmailEl.textContent = currentSettings.cloudEmail;
      } else {
        cloudSignedOut.style.display = "";
        cloudSignedIn.style.display = "none";
      }
    } else {
      modeCloudBtn.classList.remove("toggle-btn-active");
      modeBYOKBtn.classList.add("toggle-btn-active");
      cloudSection.style.display = "none";
      byokSection.style.display = "";

      // Show model & temperature in BYOK mode
      if (modelSection) modelSection.style.display = "";
    }
  }

  modeCloudBtn.addEventListener("click", () => {
    currentSettings.useCloud = true;
    updateModeUI(true);
  });

  modeBYOKBtn.addEventListener("click", () => {
    currentSettings.useCloud = false;
    updateModeUI(false);
  });

  // ── Cloud Sign In ─────────────────────────────────────────────────────

  // CLOUD_AUTH_URL imported from config.js

  cloudSigninBtn.addEventListener("click", async () => {
    // Open the auth page and poll for token via data attributes (not URL hash)
    // This avoids exposing the token in browser history
    let authTabId = null;
    let pollInterval = null;
    let cleanupTimeout = null;

    const TOKEN_REGEX = /^max_[A-Za-z0-9]{32}$/;
    ...
    async function pollForToken() {
      ...
      try {
        const results = await browser.scripting.executeScript({
          ...
          func: () => ({
            token: document.documentElement.dataset.maxToken || "",
            email: document.documentElement.dataset.maxEmail || "",
          }),
        });

        const data = results?.[0]?.result;
        if (!data?.token) return; // Not ready yet

        // Validate token format
        if (!TOKEN_REGEX.test(data.token)) return;

        // Basic email format validation if present
        if (data.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.email)) return;

        // Got the token — stop polling and save
        clearInterval(pollInterval);
        clearTimeout(cleanupTimeout);

        currentSettings.cloudToken = data.token;
        currentSettings.cloudEmail = data.email || "";
        currentSettings.useCloud = true;
        updateModeUI(true);
        ...
        showStatus("Connected to Max Cloud!", "success");

        // Close the auth tab after a short delay
        setTimeout(() => browser.tabs.remove(authTabId).catch(() => {}), 1500);
        authTabId = null;
      } catch {
        // Tab might not be ready yet or was closed — ignore
      }
    }

    const tab = await browser.tabs.create({ url: CLOUD_AUTH_URL });
    authTabId = tab.id;

    // Poll every 500ms for the token to appear in the page's data attributes
    pollInterval = setInterval(pollForToken, 500);

    // Cleanup after 5 minutes if auth hasn't completed
  ...
  cloudSignoutBtn.addEventListener("click", () => {
    currentSettings.cloudToken = "";
    currentSettings.cloudEmail = "";
    updateModeUI(true); // Stay in cloud mode but show signed out
    ...
    showStatus("Signed out from Max Cloud", "info");
  });
  ...
  saveBtn.addEventListener("click", async () => {
    const settings = {
      ...
      useCloud: currentSettings.useCloud !== false,
      cloudToken: currentSettings.cloudToken || "",
      cloudEmail: currentSettings.cloudEmail || "",
      shortcuts: {
  ...
  testBtn.addEventListener("click", async () => {
    ...
    if (settings.useCloud && !settings.cloudToken) {
      showStatus("Sign in to Max Cloud first", "error");
      return;
    }
