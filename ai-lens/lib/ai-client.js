...
async function fetchModels(settings) {
  ...
  try {
    ...
    if (format === "openai") {
      ...
      // Gemini: GET /v1beta/models — key sent via header, not URL
      url = `${baseUrl}/models`;
      headers = {
...
async function aiComplete(settings, systemPrompt, userPrompt) {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      throw new Error(toUserError(resp.status, errBody));
    }
...
/**
 * Convert a raw API error status and body into a sanitized user-facing message.
 * Prevents leaking internal error details to the UI.
 */
function toUserError(status, rawBody) {
  console.error('[Max] API error', status, rawBody);
...
export { PROVIDERS, aiComplete, fetchModels, toUserError };
